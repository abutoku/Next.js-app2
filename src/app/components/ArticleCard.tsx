  ...
  return (
    <article className='shadow my-4' key={article.id}>
      ...
      <div className="bg-white flex flex-col justify-start p-6">
        ...
        <Link href={`articles/${article.id}`} className='text-slate-900 text-3xl font-bold hover:text-gray-700'>{article.title}</Link>
          <p className='text-sm pb-3 text-slate-900'>
            Published on {new Date(article.createdAt).toLocaleDateString()}
          </p>
        <Link href={`articles/${article.id}`} className='text-slate-900 pb-6'>
          {article.content.length > 70 ? article.content.substr(0, 70) + '...' : article.content}
        </Link>
